...
class Stage2_TrAdaBoostR2:
    def __init__(self,
                 ...
                 sample_size = None,       #{源域数据个数，目标域数据个数]
                 n_estimators = 50,
                 learning_rate = 1.,
                 loss = 'linear',
    ...
    def fit(self, X, y, sample_weight=None):
        ...
        if sample_weight is None:
            ...
            sample_weight = np.empty(X.shape[0], dtype=np.float64)  #初始化为0
...
class TwoStageTrAdaBoostR2:
    ...
    def fit(self, X, y, sample_weight=None):
        ...
        for istep in range(self.steps):
            model = Stage2_TrAdaBoostR2(self.base_estimator,                                 #第s步的模型，该模型的误差由下面的K折交叉验证给出，而不是利用该训练好的模型给出
            ...
            for train, test in kf.split(X_target):            #K折交叉验证给出上面模型的误差
    ...
    def _beta_binary_search(self, istep, sample_weight, error_vect, stp):
        ...
        while np.abs(updated_weight_sum - theoretical_sum) > 0.1:
...
if __name__ == "__main__":
    ...
    a_y = np.random.randn(20)
    b = np.random.randn(10,3)
    b_y = np.random.randn(10)
    model = TwoStageTrAdaBoostR2(sample_size=(20,10))
    model.fit(np.concatenate((a,b),axis=0),np.concatenate((a_y,b_y)))
